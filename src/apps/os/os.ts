import { AppBuilder, IApp, Message, Tool, WindowWithFunctions } from "llm-os";
...
export const OSAppBuilder = AppBuilder
    .start()
    .setState(<APPS extends Record<string, IApp<any, any>>>() => ({
        opennedApps: [] as string[],
        lastAppAction: {} as Record<string, any>,
        apps: {} as APPS,
        windowsMap: {} as Record<string, WindowWithFunctions<any>>,
        goal: "" as string,
        // actIf: ((d: any) => true) as (states: { [K in keyof APPS]: APPS[K]['state'] }) => boolean
    }))
    ...
    .setButtonPressHandler(data => {
        ...
        if (data.function.name === "openApp") {
            state.opennedApps.push(((data.function.args as any).appName as string));
            state.lastAppAction[((data.function.args as any).appName as string)] = 0;
            return state;
        }
        ...
        state.opennedApps.forEach(app => {
            state.lastAppAction[app] = (state.lastAppAction[app] || 0) + 1;
        });

        const oldApps = state.opennedApps.filter(app => state.lastAppAction[app] > 5);
        oldApps.forEach(app => {
            state.opennedApps = state.opennedApps.filter(a => a !== app);
        });
